...
///   2. Register the Npgsql data source, <see cref="DbConnectionFactory"/>, and the filesystem migrator via <c>AddDatabaseBespokeMigrations</c>
...
internal static partial class CliRunner
{
    ...
    public static async Task<int> ApplyAsync(string? connection, string? sqlDir, CancellationToken ct)
    {
        ...
        // Create the target database first — a fresh box has no smartqr database yet.
    ...
    /// <param name="sqlDir">The resolved migrations directory.</param>
    public static int MergeDev(string? sqlDir)
    {
