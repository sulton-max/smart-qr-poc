...
/// <summary>Builds the <c>smart-qr-migrate</c> command tree — the global options and the status, apply, rollback, verify, new, and merge verbs.</summary>
/// <remarks>
/// Tree shape:
///   root (--connection, --sql-dir recursive)
///     ├─ status · apply · rollback (--to) · verify (--repair) · new &lt;name&gt; · merge
...
internal static class CliCommands
{
    ...
    public static RootCommand Build()
    {
        ...
        {
            Description = "Postgres connection (else SMARTQR_DB_CONNECTION, else localhost/smartqr).",
            Recursive = true,
        };

        var sqlDirOption = new Option<string?>("--sql-dir")
        {
            Description = "Migrations dir (else SMARTQR_SQL_DIR, else auto-discovered).",
            Recursive = true,
        };

        var root = new RootCommand("smart-qr-migrate — SQL migrator CLI (dev).");
        ...
        root.Subcommands.Add(BuildMerge(sqlDirOption));
    ...
    /// <summary>Builds the <c>merge</c> verb — promote <c>Dev/*.sql</c> drafts into numbered migrations.</summary>
    /// <param name="sqlDir">The global sql-dir option to read in the action.</param>
    private static Command BuildMerge(Option<string?> sqlDir)
    {
        var command = new Command("merge", "Promote Dev/*.sql → numbered NNN-name/{Apply,Rollback}.sql.");
        command.SetAction((parseResult, _) =>
            Task.FromResult(CliRunner.MergeDev(parseResult.GetValue(sqlDir))));
        return command;
    }
}
