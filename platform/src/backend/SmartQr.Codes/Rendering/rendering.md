...
1. `SkiaSharp.NativeAssets.Linux` — referenced in `SmartQr.Codes.csproj` (ships `libSkiaSharp.so` for linux-x64/arm64/musl into any Linux publish). Pinned to the SkiaSharp version `Svg.Skia` drags in (3.119.2).
...
Verified: a framework-dependent linux-x64 publish runs the full `SvgRenderer → SkiaSvgRasterizer` path and emits a valid PNG inside `dotnet/aspnet:10.0` once `libfontconfig1` is present. No Dockerfile exists in this repo yet — add the line above when one is created.
