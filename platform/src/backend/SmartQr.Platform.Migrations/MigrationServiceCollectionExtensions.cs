...
public static class MigrationServiceCollectionExtensions
{
    ...
    /// <remarks>The host passes the assembly that embeds the <c>Migrations/NNN-name/*.sql</c> resources; requires <see cref="DbConnectionFactory"/> from <c>AddSmartQrPersistence</c>.</remarks>
    ...
    /// <remarks>Wire from the CLI host; reads <c>{root}/NNN-name/{Apply,Rollback}.sql</c> and requires <see cref="DbConnectionFactory"/> registered alongside.</remarks>
    /// <param name="services">The service collection to register into.</param>
        ...
        this IServiceCollection services, Func<IServiceProvider, IMigrationSource> sourceFactory, Action<MigrationOptions>? configure)
    {
        ...
        services.AddSingleton(CreateDialect(options.Provider));
