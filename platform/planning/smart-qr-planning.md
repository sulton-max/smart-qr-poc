...
- **2026-06-23:** v0.4 **Iter 10 follow-on — SDK `10.0.37-beta` (test-DB switch + host/EF/JSON gaps) rolled across all 3 apps.** Published the **test-DB provider switch** (`TestSetupOptions.Current.Database`; `RelationalTestDb<T>` — PG default = container+Respawn, SQLite = in-mem recreate; whole suite flipped by a `[ModuleInitializer]`) + 4 **gap extractions** to the SDK source (`AddPostgresPersistence<T>`+`MigrateBespokeOnStartupAsync` host floor · `ApplyEnumStringConversions` · SQLite-gated `ApplyDateTimeOffsetToBinaryConversion` · `AddJsonStringEnums`). **Adopted across smart-qr · drydock · secrets-vault** (3 parallel agents, disjoint repos): each bumped to `10.0.37-beta`, swapped local test-DB/migrator forks → the SDK harnesses, host wiring → `AddPostgresPersistence`, `OnModelCreating` → `ApplyEnumStringConversions`, controllers → `AddJsonStringEnums`; each keeps its **own** error model (no SDK error-layer adoption — separate in-flight chat). **Green, both providers:** smart-qr PG 88 (52 unit · 26 E2E · 10 migrator) + SQLite 52/52 · drydock PG 115 + SQLite 11 · secrets-vault PG 2 + SQLite 2. drydock deleted its `Migrations.Tests/Harness` migrator fork → SDK `MigratorHarness`; secrets-vault (12-ver jump from `10.0.25`) aliased a new SDK `Mediator.Result.FailureCategory` collision to pin its product type. **Cross-app gotcha:** `AddPostgresPersistence` eager-resolves the conn-string at registration (before `WebApplicationFactory.ConfigureAppConfiguration`) → E2E/HTTP fixtures must set the `DB_CONNECTION` env var **before** host build. No commits (manual push).
