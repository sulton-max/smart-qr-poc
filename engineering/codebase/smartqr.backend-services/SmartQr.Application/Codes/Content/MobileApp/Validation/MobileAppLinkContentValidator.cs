...
/// overridden to the wire field keys (<c>appStore</c> / <c>playStore</c> / <c>other</c>) so a failure points the client at the right input.
/// </summary>
public sealed class MobileAppLinkContentValidator : AbstractValidator<MobileAppLinkContent>
{
    ...
    public MobileAppLinkContentValidator()
    {
        ...
        Link(model => model.AppStore, "appStore", "App Store link");
        Link(model => model.PlayStore, "playStore", "Google Play link");
